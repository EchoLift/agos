---
title: What is AGOS?
description: Understand how AGOS helps an agency run client content work.
category: Getting Started
order: 1
roles:
  - OWNER
  - MANAGER
status: AVAILABLE
---

## What It Solves

AGOS replaces the daily agency mess of WhatsApp follow-ups, spreadsheets, scattered Drive links, and unclear ownership.

It is built around one simple idea: every piece of work should have an owner, a deadline, a current state, and a clear next action.

## How The Product Thinks

AGOS has two ways to run work:

- **Campaigns** for structured client plans such as 8 reels per week.
- **Gigs** for direct assignments such as 5 scripts by Friday.

Both eventually appear in My Work, Workflow, and Calendar so employees do not need to understand the whole agency to finish their tasks.

## What Happens Next

Start by creating an agency, adding your team, adding a client, then choosing either a campaign or a gig.

> [!IMPORTANT] Current Status
> Campaigns, gigs, workflow, calendar, team, roles, and client playbooks are available. Client portal, WhatsApp delivery, Google Calendar sync, and media hosting are not available yet.
