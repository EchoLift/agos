...
CLIENT is different from normal internal agency roles because it can be associated with a specific business client.

A CLIENT-only user is restricted to the business client associated with their membership.

If a member also has legitimate internal agency roles, such as:
...
The CLIENT association does not globally reduce an internal team member to client-only access.

## Assigning The CLIENT Role

When adding CLIENT to an existing member, you must select the business client they represent.

Because the person already belongs to the agency:

- Their existing membership is updated.
- The selected business client is associated with that membership.
- No new invitation is created.
- No invitation email is sent.

If CLIENT is later removed and no longer required, the client association is cleared.

> [!TIP] Model Real Responsibilities
> Give people the roles that reflect what they actually do. Use assignments to decide which specific campaigns, gigs, and workflow tasks they are responsible for.