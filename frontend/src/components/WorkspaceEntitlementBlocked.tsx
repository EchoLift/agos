...
import { getCentralAppHref, getWorkspaceUrl } from "@/lib/workspace-url";
import { hasBillingRole } from "@/lib/billing-ui";

export default function WorkspaceEntitlementBlocked({
  agency,
}: {
  agency: Agency;
}) {
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#09090b] px-6 text-zinc-100">
      <section className="w-full max-w-xl rounded-3xl border border-amber-400/20 bg-zinc-950 p-8 shadow-2xl">
        ...
        <h1 className="mt-4 text-3xl font-semibold">
          AGENCIE access is not active for this organisation.
        </h1>
        ...
        {isAdmin && (
          <dl className="mt-6 grid gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 text-sm">
            <div className="flex justify-between gap-4">
              ...
              <dd>{entitlement?.status ?? "Not configured"}</dd>
            </div>
            {entitlement?.plan && (
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500">Plan</dt>
                <dd>{entitlement.plan}</dd>
              </div>
            )}
            {expiry && (
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500">Access end</dt>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {hasBillingRole(agency) ? (
            ...
            <span className="text-sm text-zinc-500">
              Contact your Owner or Finance team to restore access.
            </span>
          )}
          {data?.agencies
            ...
            .map((item) => (
              <a
                key={item.id}
                href={getWorkspaceUrl(item.slug)}
                className="rounded-xl border border-zinc-700 px-4 py-2.5 text-sm"
              >
                Switch to {item.displayName || item.name}
              </a>
            ))}
        </div>
      </section>
