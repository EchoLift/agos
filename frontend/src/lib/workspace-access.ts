...
const navByKey = new Map(
  workspaceNavItems.map((item) => [item.key, item]),
);
...
const isRoleTestingOverrideEnabled =
  ...
  process.env
    .NEXT_PUBLIC_DEV_ROLE_TESTING_OVERRIDE_ENABLED ===
    "true";
...
export function allowedNavKeys(
  ...
  roleKeys.forEach((roleKey) => {
    roleAccess[roleKey]?.forEach((navKey) =>
      keys.add(navKey),
    );
  });
...
export function roleAccessLabels(
  ...
  return access
    .map((item) => navByKey.get(item)?.label)
    .filter(
...
export function canAccessWorkspacePath(
  ...
  if (
    relativePath.startsWith("/settings/profile") ||
    relativePath.startsWith("/settings/status") ||
    relativePath.startsWith("/settings/appearance")
  ) {
    return true;
  }

  if (relativePath.startsWith("/settings/agency")) {
    return hasAnyRole(agency, ["OWNER", "ADMIN"]);
  }

  if (relativePath === "/campaigns/new") {
    return hasAnyRole(agency, [
      "OWNER",
      "ADMIN",
      "MANAGER",
    ]);
  ...
  if (relativePath.startsWith("/gigs")) {
    return allowed.has("gigs");
  }
...
export function hasAnyRole(
  ...
  const normalized = new Set(
    roleKeys.map(normalizeRoleKey),
  );

  return getAgencyRoleKeys(agency).some((roleKey) =>
    normalized.has(roleKey),
  );
}

export function isBusinessDashboardRole(
  agency: Agency | null,
) {
  return hasAnyRole(agency, [
    "OWNER",
...
export function isProductionWorkspaceRole(
  agency: Agency | null,
) {
  return (
    hasAnyRole(agency, [
      ...
      "SOCIAL_MEDIA_MANAGER",
      "MEMBER",
    ]) && !isBusinessDashboardRole(agency)
  );
}

export function workspaceHomeLabel(
  agency: Agency | null,
) {
  return isProductionWorkspaceRole(agency)
    ? "My Work"
    : "Dashboard";
}

export function workspaceHomeShortLabel(
  agency: Agency | null,
) {
  return isProductionWorkspaceRole(agency)
    ? "MW"
...
export function getAgencyRoleKeys(
  agency: Agency | null,
) {
  const rawRoles = [
    agency?.role,
    ...(
      agency?.roles?.flatMap((role) => [
        role.key,
        role.name,
      ]) ?? []
    ),
  ...
  return Array.from(
    new Set(rawRoles.map(normalizeRoleKey)),
  );
}
