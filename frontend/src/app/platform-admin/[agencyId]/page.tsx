...
export default function PlatformAgencyPage({
  ...
  const query = useQuery({
    queryKey: ["platform-admin", "agency", agencyId],
    queryFn: () => getPlatformAgency(agencyId),
  });
  ...
  if (!query.data)
    return (
      <Shell>
        <p className="text-zinc-400">Loading agency…</p>
      </Shell>
    );
  const { agency, metrics, members, recentActivity } = query.data;
  return (
    <Shell>
      <Link href="/platform-admin" className="text-sm text-indigo-300">
        ← Platform overview
      </Link>
      <div className="mt-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold">
            {agency.displayName || agency.name}
          </h1>
        ...
        <p className="rounded-full border border-zinc-700 px-3 py-1 text-sm">
          {agency.subscription?.status ?? "No entitlement"}
        </p>
      </div>
      ...
      <EntitlementEditor
        key={agency.subscription?.updatedAt ?? "new"}
        agencyId={agencyId}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
          ...
          <div className="mt-4 space-y-3">
            {members.map((member) => (
              <div
                key={member.id}
                className="flex justify-between border-t border-zinc-900 pt-3 text-sm"
              >
                <span>{member.user.name || "Unnamed user"}</span>
                <span className="text-zinc-500">
                  {member.role.systemRole.key}
                </span>
              </div>
            ))}
          </div>
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
          <h2 className="font-semibold">Meaningful activity</h2>
          <div className="mt-4 space-y-3">
            {recentActivity.length ? (
              recentActivity.map((event) => (
                <div
                  key={event.id}
                  className="border-t border-zinc-900 pt-3 text-sm"
                >
                  <p>{event.eventType}</p>
              ...
              <p className="text-sm text-zinc-500">
                Unavailable from current audit records.
              </p>
            )}
          </div>
        </section>
...
function EntitlementEditor({
  ...
  const [reactivationStatus, setReactivationStatus] =
    useState<SubscriptionStatus>("ACTIVE");
  const [reactivationTrialEnd, setReactivationTrialEnd] = useState(
    initial.trialEndsAt,
  );
  ...
  async function confirmAction(action: "SUSPEND" | "CANCEL") {
    ...
    const confirmed = await dialog.confirm({
      title: cancelling
        ? `Cancel AGENCIE access for ${agencyName}?`
        : `Suspend AGENCIE access for ${agencyName}?`,
    ...
    await runConfirmedEntitlementAction({
      ...
      payload: entitlementActionPayload(
        subscription,
        cancelling ? "CANCELLED" : "SUSPENDED",
      ),
  ...
  return (
    <>
      <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
        <h2 className="font-semibold">Manual entitlement</h2>
        ...
        <button
          ...
          className="mt-5 rounded-xl bg-indigo-500 text-white px-5 py-3 text-md font-semibold disabled:opacity-50"
        >
          {mutation.isPending ? "Saving…" : "Save entitlement"}
        </button>
        {validationError && (
        ...
        {mutation.isSuccess && (
          <p className="mt-3 text-sm text-emerald-300">
            Entitlement updated and audited.
          </p>
        )}
      </section>
      {actions.length > 0 && (
        <section className="mt-4 rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
          <h2 className="font-semibold">Entitlement actions</h2>
          <p className="mt-2 text-sm text-zinc-500">
            Operational shortcuts preserve agency data, memberships, and
            entitlement history.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            {actions.includes("SUSPEND") && (
              <button
                disabled={mutation.isPending}
                onClick={() => void confirmAction("SUSPEND")}
                className="rounded-xl border border-amber-700/30 bg-amber-500/10 px-4 py-2.5 text-sm font-semibold text-amber-900"
              >
                Suspend access
              </button>
            )}
            ...
            {actions.includes("CANCEL") && (
              <button
                ...
                className="rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-2.5 text-sm font-semibold text-red-300"
              >
                Cancel access
              </button>
            )}
          ...
          {reactivating && (
            <div className="mt-5 rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
              <p className="text-sm text-zinc-300">
                Choose how to reactivate. No billing action will be created.
              </p>
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                ...
                {reactivationStatus === "TRIAL" && (
                  <label className="text-sm text-zinc-400">
                    New trial end
                    <input
                      type="datetime-local"
                      value={reactivationTrialEnd}
                      onChange={(e) => setReactivationTrialEnd(e.target.value)}
                      className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-white"
                    />
                  </label>
                )}
                {reactivationStatus === "ACTIVE" && (
                  <label className="text-sm text-zinc-400">
                    Subscription end
                    <input
                      type="datetime-local"
                      value={reactivationEnd}
                      onChange={(e) => setReactivationEnd(e.target.value)}
                      className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-white"
                    />
                  </label>
                )}
