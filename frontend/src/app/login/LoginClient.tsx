...
export default function LoginClient() {
    ...
    const redirectAfterLogin = useCallback(
        async () => {
            ...
            const currentAgency =
                getCurrentAgency(memberships);

            if (currentAgency) {
                window.location.href =
                    getWorkspaceUrl(currentAgency.slug);
                return;
            }
    ...
    useEffect(() => {
        ...
        if (
            !container ||
            googleButtonMountedRef.current
        ) {
            return;
        }
                ...
                <div className="w-full rounded-3xl border border-zinc-800 bg-zinc-950/80 p-10 shadow-2xl shadow-black/30">
                    <div className="mb-8">
                        ...
                        <p className="mt-3 max-w-xl text-sm leading-7 text-zinc-400">
                            One click and we&apos;ll take you straight
                            to your workspace. No forms, no passwords.
                        </p>
                    </div>
                    ...
                    <p className="mt-6 text-center text-sm text-zinc-500">
                        Secure authentication via Google and the
                        AGENCIE backend session.
                    </p>
                </div>
            </main>
        </div>
