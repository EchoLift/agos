...
import { CalendarEvent, CalendarScope } from "@/lib/api/calendar";
...
export default function CalendarPage() {
  ...
  const roleKeys = useMemo(
    () =>
      [agency?.role, ...(agency?.roles?.map((role) => role.key) || [])].filter(
        Boolean,
      ),
    [agency],
  );
  ...
  const [visibleTypes, setVisibleTypes] =
    useState<string[]>(defaultVisibleTypes);
  const [mobileRepresentation, setMobileRepresentation] =
    useState<MobileRepresentation>("calendar");
  ...
  const selectedScope = allowedScopeOptions.some(
  ...
  const isFirstCalendarLoad = calendarQuery.isLoading && !calendar;
  const calendarError =
    !calendar && calendarQuery.error
      ...
      : null;
  const visibleTypesStorageKey = useMemo(
    () => agencyStorageKey(agencyId, "calendar.visibleTypes"),
    [agencyId],
  );
  const visibleTypesVersionKey = useMemo(
    () => agencyStorageKey(agencyId, "calendar.visibleTypesVersion"),
    [agencyId],
  );
  const mobileRepresentationKey = useMemo(
    () =>
      agencyStorageKey(
        agencyId,
        `calendar.${agency?.membershipId || "member"}.mobileRepresentation`,
      ),
  ...
  const mobileModeKey = useMemo(
    () =>
      agencyStorageKey(
        agencyId,
        `calendar.${agency?.membershipId || "member"}.mobileMode`,
      ),
    [agency?.membershipId, agencyId],
  );
  ...
  useEffect(() => {
    ...
    const stored =
      window.localStorage.getItem(visibleTypesStorageKey) ?? legacyStored;
    if (stored) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          const missingDefaultTypes = hasCurrentDefaults
            ? []
            : defaultVisibleTypes.filter(
                (type) => !restoredTypes.includes(type),
              );
          queueMicrotask(() =>
  ...
  useEffect(() => {
    window.localStorage.setItem(
      ...
      JSON.stringify(visibleTypes),
    );
  }, [visibleTypes, visibleTypesStorageKey]);

  useEffect(() => {
    const storedRepresentation = window.localStorage.getItem(
      mobileRepresentationKey,
    );
    const storedMode = window.localStorage.getItem(mobileModeKey);
    queueMicrotask(() => {
      if (
        storedRepresentation === "calendar" ||
        storedRepresentation === "agenda"
      ) {
        setMobileRepresentation(storedRepresentation);
      }
      if (
        storedMode === "day" ||
        storedMode === "three-day" ||
        storedMode === "week" ||
        storedMode === "month"
      ) {
  ...
  const groupedEvents = useMemo(
    () => groupEvents(displayedEvents),
    [displayedEvents],
  );
  const calendarEvents = useMemo(
    () => toIlamyEvents(displayedEvents),
    [displayedEvents],
  );
  const clients = useMemo(
    () => uniqueClients(calendar?.events || []),
    [calendar],
  );
  const statuses = useMemo(
    () => uniqueValues((calendar?.events || []).map((event) => event.status)),
    [calendar],
  );
  const mobileSummary = useMemo(
    () => ({
      total: displayedEvents.length,
      publishing: displayedEvents.filter(
        (event) => event.eventType === "PUBLISHING",
      ).length,
      overdue: displayedEvents.filter((event) => event.riskStatus === "OVERDUE")
        .length,
    }),
    [displayedEvents],
  );
  const googleCalendarHref = useMemo(
    () =>
      agencySlug
        ? getWorkspaceHref(agencySlug, "/settings/profile#google-calendar")
  ...
  const renderVisualCalendarEvent = useCallback(
    (event: IlamyCalendarEvent) => <VisualCalendarEvent event={event} />,
    [],
  );
  const handleVisualCalendarEventClick = useCallback(
    ...
    <div className="flex h-[calc(100dvh_-_8.75rem_-_env(safe-area-inset-bottom))] min-h-0 flex-col gap-2 overflow-hidden lg:block lg:h-auto lg:overflow-visible lg:space-y-5">
      <div className="hidden flex-wrap items-end justify-between gap-3 lg:flex">
        ...
        <button
          ...
          className="min-h-11 rounded-md border border-zinc-800 px-3 text-sm font-semibold text-zinc-300 transition hover:bg-zinc-900 lg:rounded-full"
        >
          Reset to defaults
        </button>
      </div>
              ...
              <p className="mt-1 hidden text-sm text-zinc-400 sm:block">
                {isGoogleCalendarConnected
                  ...
                  : googleCalendarStatusQuery.isLoading
                    ? "Checking your Google Calendar sync status."
                    : "See your AGENCIE tasks outside AGENCIE."}
              </p>
            </div>
          </div>
          <Link
        ...
        <main className="flex min-w-0 flex-col gap-3 lg:gap-4">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:gap-3">
            ...
            <Metric
              ...
              value={calendar?.summary.assignedToMe || 0}
            />
            <Metric
              label="Publishing"
              value={calendar?.summary.publishing || 0}
            />
            <Metric
              label="Overdue"
              value={calendar?.summary.overdue || 0}
              tone="danger"
            />
          </div>
          ...
          <section className="order-3 hidden rounded-xl border border-zinc-800 bg-zinc-950/80 p-3 shadow-xl shadow-black/10 sm:block lg:order-2 lg:p-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3 px-1">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Week, day, month, and year views use the same role-aware
                  events.
                </p>
              </div>
            ...
            <div className="h-[560px] overflow-hidden rounded-lg border border-zinc-800 bg-background lg:h-[720px]">
              <IlamyCalendar
                ...
                disableDragAndDrop
                eventHeight={42}
                onEventClick={handleVisualCalendarEventClick}
                renderEvent={renderVisualCalendarEvent}
              />
            </div>
          </section>
          ...
          <section className="order-2 rounded-lg border border-zinc-800 bg-zinc-950/80 p-3 shadow-xl shadow-black/10 sm:order-3 lg:rounded-xl lg:p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  {scopeLabel(selectedScope)} · {formatDate(range.from)} to{" "}
                  {formatDate(range.to)}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3">
              {calendarQuery.isError && calendar ? (
                <div className="mb-3 rounded-md border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
                  Calendar refresh failed. Showing the last loaded schedule.
                </div>
              ) : null}
              {isFirstCalendarLoad ? (
          ...
          <div className="absolute inset-x-0 bottom-0 max-h-[88dvh] overflow-y-auto rounded-t-lg border border-zinc-800 bg-zinc-950 shadow-2xl">
            <div className="sticky top-0 z-10 flex min-h-14 items-center justify-between border-b border-zinc-800 bg-zinc-950 px-4">
              ...
              <button
                ...
                onClick={() => setIsFilterOpen(false)}
                aria-label="Close filters"
                className="grid h-11 w-11 place-items-center rounded-md text-zinc-300"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="sticky bottom-0 flex gap-2 border-t border-zinc-800 bg-zinc-950 p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
              <button
                type="button"
                onClick={resetDefaults}
                className="min-h-11 flex-1 rounded-md border border-zinc-700 text-sm font-semibold text-zinc-200"
              >
                Reset
              </button>
              <button
                type="button"
                onClick={() => setIsFilterOpen(false)}
                className="min-h-11 flex-1 rounded-md bg-indigo-500 text-sm font-semibold text-white"
              >
                Apply
              </button>
            </div>
    ...
    <div className="space-y-3">
      <div>
        ...
        <select
          value={selectedScope}
          onChange={(event) =>
            onScopeChange(event.target.value as CalendarScope)
          }
      ...
      <div className="grid gap-2">
        ...
        <input
          ...
          onChange={(event) =>
            onRangeChange((current) => ({
              ...current,
              to: endOfDay(event.target.value).toISOString(),
            }))
          }
          className="date-input min-h-11 rounded-md border border-zinc-800 bg-[#0b0b11] px-3 text-base text-white outline-none focus:border-indigo-500 lg:text-sm"
        />
      ...
      <div>
        ...
        <div className="mt-2 space-y-1">
          {calendarTypes.map((type) => (
            <label
              key={type.value}
              className="flex min-h-11 items-center gap-3 rounded-md px-2 text-sm text-zinc-300 transition hover:bg-zinc-900/50"
            >
              <input
                type="checkbox"
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={statusPillClasses(event.eventType)}>
              {formatLabel(event.eventType)}
            </span>
            <span className={statusPillClasses(event.riskStatus)}>
              {formatLabel(event.riskStatus)}
            </span>
            {event.forwardedToMe ? (
              <span className={statusPillClasses("ASSIGNED")}>
                Assigned to me
              </span>
            ) : null}
          </div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            {formatTime(event.startsAt)} · {event.client?.name || "No client"} ·{" "}
            {event.campaign?.name || "Standalone gig"}
          </p>
          {event.contentAsset ? (
            <p className="mt-2 text-sm text-zinc-300">
              {event.contentAsset.displayCode} · {event.contentAsset.title}
            </p>
          ) : null}
          {event.workOrder ? (
            <p className="mt-2 text-sm text-zinc-300">
              {formatLabel(event.workOrder.workType)} · {event.workOrder.title}
            </p>
          ) : null}
        </div>
        <div className="text-right text-sm text-zinc-500">
          <div className={statusPillClasses(event.status)}>
            {formatLabel(event.status)}
          </div>
          <div className="mt-1">
            {event.owner?.name || event.platform || event.reason}
          </div>
        </div>
      </div>
    </button>
...
function VisualCalendarEvent({ event }: { event: IlamyCalendarEvent }) {
  ...
  return (
    <div
      className={`h-full min-h-7 overflow-hidden rounded-md border px-2 py-1 text-left text-[11px] leading-tight ${eventTypeBorderClass(type)} ${risk === "OVERDUE" || risk === "BLOCKED" ? "bg-red-500/15" : "bg-indigo-500/15"}`}
    >
      <div className="truncate font-semibold text-white">{event.title}</div>
      <div className="truncate text-zinc-300">
        {agosEvent?.contentAsset?.displayCode ||
          agosEvent?.workOrder?.title ||
          formatLabel(type)}
...
function Metric({
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-950/80 p-3 shadow-xl shadow-black/10 lg:rounded-xl lg:p-4">
      ...
      <div
        className={`mt-1 text-2xl font-semibold lg:text-3xl ${tone === "danger" ? "text-red-300" : "text-white"}`}
      >
        {value}
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 4 }, (_, index) => (
        <div
          key={index}
          className="rounded-md border border-zinc-800 bg-[#0b0b11] p-3 lg:p-4"
        >
...
function isDateInRange(date: Date, range: { from: string; to: string }) {
  ...
  return (
    timestamp >= new Date(range.from).getTime() &&
...
function uniqueClients(events: CalendarEvent[]) {
  ...
  return Array.from(clients, ([id, name]) => ({ id, name })).sort(
...
function scopeLabel(scope: CalendarScope) {
  return (
    scopeOptions.find((option) => option.value === scope)?.label || "Calendar"
  );
}
