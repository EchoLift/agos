...
export class ContentController {
  ...
  @Post()
  create(@Body() dto: CreateContentAssetDto, @CurrentUser() user: IdentityContext) {
  ...
  @Patch(':id')
  update(@Param('id') id: string, @Body() dto: UpdateContentAssetDto, @CurrentUser() user: IdentityContext) {
    return this.contentService.update(id, dto, user.agencyId ?? '', user.userId);
  }
