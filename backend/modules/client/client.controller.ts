...
export class ClientController {
  ...
  @Post()
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Client playbook created' })
  ...
  @Get(':id')
  @ApiResponse({ status: HttpStatus.OK, description: 'Role-aware client playbook' })
  findById(@Param('id') id: string, @CurrentUser() user: IdentityContext) {
    return this.clientService.findById(id, user.agencyId, user);
  }

  @Patch(':id')
  @ApiResponse({ status: HttpStatus.OK, description: 'Client playbook updated' })
  update(@Param('id') id: string, @Body() dto: UpdateClientDto, @CurrentUser() user: IdentityContext) {
  ...
  assignManager(@Param('id') id: string, @Body('membershipId') membershipId: string, @CurrentUser() user: IdentityContext) {
    return this.clientService.assignManager(id, membershipId, user.agencyId ?? '', user.userId);
  }
}
