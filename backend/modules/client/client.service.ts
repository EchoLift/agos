...
type ClientRecord = NonNullable<Awaited<ReturnType<PrismaService['client']['findUnique']>>>;
...
const CLIENT_SECTIONS: Array<{
...
}> = [
  {
    ...
    roles: ['OWNER', 'ADMIN', 'MANAGER', 'WRITER', 'DOP', 'EDITOR', 'DESIGNER', 'MEMBER'],
    fields: [
...
export class ClientService {
  ...
  async update(id: string, dto: UpdateClientDto, agencyId: string, actorId?: string) {
  ...
  async assignManager(id: string, membershipId: string, agencyId: string, actorId?: string) {
    const existing = await this.prisma.client.findUnique({ where: { id } });
    if (!existing || existing.agencyId !== agencyId) {
      throw new NotFoundException('Client not found');
    }
    ...
    await this.eventBus.publish(DomainEvents.ClientManagerAssigned, {
      agencyId,
      actorId: actorId ?? null,
      payload: { clientId: client.id, assignedManagerMembershipId: membershipId },
    });
  ...
  private optionalClientData(dto: Partial<CreateClientDto>) {
    ...
    textFields.forEach((field) => {
      if (dto[field] !== undefined) {
        data[field] = this.nullIfBlank(dto[field] as string | null | undefined) ?? null;
      }
    });
  ...
  private toVisiblePlaybook(client: ClientRecord, actor?: IdentityContext) {
    const roleKeys = new Set([...(actor?.roles ?? []), actor?.role].filter(Boolean) as string[]);
    const permissionKeys = new Set(actor?.permissions ?? []);
    const isOwner = roleKeys.has('OWNER');
    const canEdit = isOwner || permissionKeys.has('CLIENT_UPDATE') || roleKeys.has('MANAGER') || roleKeys.has('ADMIN');

    const visibleSections = CLIENT_SECTIONS.filter((section) => {
      if (isOwner) return true;
      return section.roles.some((role) => roleKeys.has(role)) || permissionKeys.has(section.permission);
    });

    const allowedFields = new Set<keyof ClientRecord>(['id', 'name', 'status', 'createdAt', 'updatedAt']);
    visibleSections.forEach((section) => section.fields.forEach((field) => allowedFields.add(field.key)));

    const visibleClient = Object.fromEntries(
      Object.entries(client).filter(([key]) => allowedFields.has(key as keyof ClientRecord)),
    );
    ...
    return {
      ...
      canEdit,
      visiblePermissions: Array.from(new Set(visibleSections.map((section) => section.permission))),
    };
  }
