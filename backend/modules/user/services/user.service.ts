...
export class UserService {
  ...
  async provisionUser(authUserId: string, profile?: { name?: string | null; avatarUrl?: string | null }): Promise<User> {
    try {
      ...
      if (error.code === 'P2002') {
        this.logger.warn(`User profile already exists for authUserId: ${authUserId}`);
  ...
  async updateProfile(userId: string, dto: UpdateProfileDto) {
    ...
    const user = await this.prisma.user.update({
      ...
      data: {
        ...
        ...(dto.avatarUrl !== undefined ? { avatarUrl: this.nullIfBlank(dto.avatarUrl) } : {}),
        ...(dto.mobileNumber !== undefined
          ? {
              mobileNumberEncrypted: mobileNumber ? this.encrypt(mobileNumber) : null,
              mobileNumberHash: mobileNumber ? this.hashLookup(mobileNumber) : null,
            }
          : {}),
        ...(dto.timezone !== undefined ? { timezone: this.nullIfBlank(dto.timezone) } : {}),
        ...(dto.language !== undefined ? { language: this.nullIfBlank(dto.language) } : {}),
        ...(dto.jobTitle !== undefined ? { jobTitle: this.nullIfBlank(dto.jobTitle) } : {}),
  ...
  async clearStatus(userId: string) {
    ...
    return { presenceStatus: PresenceStatus.AVAILABLE, workLocation: null, statusMessage: null, statusExpiresAt: null };
  }
  ...
  private toProfileResponse(user: User & { authUser?: { emailEncrypted: string } }) {
    return {
      id: user.id,
      name: user.name,
  ...
  private decryptOptional(value?: string | null) {
    ...
    try {
      ...
      const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(this.encryptionSecret(), 'hex'), Buffer.from(ivHex, 'hex'));
  ...
  private encryptionSecret() {
    const secret = this.configService.get<string>('ENCRYPTION_SECRET') ?? this.configService.get<string>('FIELD_ENCRYPTION_KEY_BASE64');
