...
import { CampaignStatus, ClientStatus, ContentAssetStatus, WorkflowInstanceStatus } from '@prisma/client';
import { PrismaService } from '@packages/database/prisma.service';

type ActivationStep = 'agency' | 'team' | 'client' | 'campaign' | 'content' | 'workflow';
type NextStep = 'CREATE_CLIENT' | 'CREATE_CAMPAIGN' | 'CREATE_CONTENT' | 'START_WORKFLOW' | null;
...
export class ActivationService {
  ...
  async getActivation(agencyId: string) {
    const [
      ...
      this.prisma.membership.count({ where: { agencyId, status: 'ACTIVE', deletedAt: null } }),
      this.prisma.client.count({ where: { agencyId, status: ClientStatus.ACTIVE, deletedAt: null } }),
    ...
    const progress = Math.round((stepValues.filter(Boolean).length / stepValues.length) * 100);
    const completed = steps.agency && steps.client && steps.campaign && steps.content && steps.workflow;
