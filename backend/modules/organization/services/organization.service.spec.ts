...
describe('OrganizationService Unit Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const mockRequestContext = {
      get: jest.fn().mockReturnValue({ correlationId: 'corr-123', requestId: 'req-123' }),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: PrismaService, useValue: { systemRole: { upsert: jest.fn() }, permission: { upsert: jest.fn() }, systemRolePermission: { upsert: jest.fn() } } },
  ...
  describe('createAgency', () => {
    it('should create an agency and return owner membership', async () => {
      ...
      const mockAgency = { id: 'agency-1', name: 'sunrise-media', displayName: 'Sunrise Media', slug: 'sunrise-media' } as Agency;
      const mockMembership = { id: 'mem-1', agencyId: 'agency-1', userId: 'user-1' } as Membership;
      ...
      const result = await service.createAgency({ displayName: 'Sunrise Media', slug: 'sunrise-media' }, 'auth-user-1', 'session-1');
    ...
    it('should throw NotFoundException if user profile does not exist', async () => {
      ...
      await expect(service.createAgency({ displayName: 'Sunrise Media', slug: 'sunrise-media' }, 'auth-user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('getMyMemberships', () => {
    it('should return active agency and list of memberships', async () => {
      ...
      repository.findMembershipsByUserId.mockResolvedValue([
        {
          id: 'mem-1',
          agency: { id: 'agency-1', name: 'sunrise-media', displayName: 'Sunrise Media', slug: 'sunrise-media' },
          role: { id: 'role-owner', displayName: 'Owner', systemRole: { key: 'OWNER' } },
          roles: [{ role: { id: 'role-owner', displayName: 'Owner', systemRole: { key: 'OWNER' } } }],
        },
      ]);
  ...
  describe('inviteMember', () => {
    it('should create an invitation for valid inviter and role', async () => {
      ...
      const mockInviterMem = { id: 'mem-inviter', agencyId: 'agency-1' } as Membership;
      const mockRole = { id: 'role-manager', displayName: 'MANAGER', systemRole: { key: 'MANAGER' } } as unknown as Role;
      ...
      expect(cryptoService.hashLookup).toHaveBeenCalledWith('editor@example.com');
    });
    ...
    it('should throw ForbiddenException if inviter is not a member of agency', async () => {
      ...
      await expect(
        service.inviteMember('agency-1', { email: 'e@test.com', roleId: 'r-1' }, 'auth-1'),
  ...
  describe('acceptInvitation', () => {
    it('should accept invitation and return new membership', async () => {
      ...
      const mockMembership = { id: 'mem-new', agencyId: 'agency-1', status: 'ACTIVE' } as Membership;
    ...
    it('should throw ConflictException if user is already a member', async () => {
      ...
      repository.findInvitationByToken.mockResolvedValue({ id: 'inv-1', agencyId: 'agency-1' } as Invitation);
      repository.findMembership.mockResolvedValue({ id: 'mem-existing' } as Membership);

      await expect(service.acceptInvitation('token-123', 'auth-2')).rejects.toThrow(ConflictException);
    });
  });
});
