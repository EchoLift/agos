...
export class AuthController {
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Validation failed or email in use' })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Returns access token and sets refresh token cookie' })
  ...
  async google(@Body() dto: GoogleOAuthDto, @Res({ passthrough: true }) res: Response) {
    const { accessToken, refreshToken } = await this.authService.googleLogin(dto.token);
    this.setRefreshTokenCookie(res, refreshToken);
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Returns new access token and updates refresh token cookie' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Invalid or missing refresh token' })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Session revoked and cookie cleared' })
