...
describe('AuthService Integration', () => {
  ...
  beforeEach(async () => {
    ...
    const mockRequestContext = {
      get: jest.fn().mockReturnValue({ correlationId: 'test-corr-id', requestId: 'test-req-id', ip: '127.0.0.1' }),
    };
    const mockUserService = {
      provisionUser: jest.fn().mockResolvedValue({ id: 'profile-user-id', authUserId: 'auth-user-id' }),
    };
  ...
  it('should detect token reuse and revoke the family', async () => {
    ...
    await expect(authService.refresh(providedRefreshToken))
      .rejects
    ...
    expect(repository.revokeFamily).toHaveBeenCalledWith(
      'family-id',
      expect.objectContaining({ reason: 'Token Reuse Detected or Expired Token' }),
      'test-corr-id'
    );
  });
  ...
  it('should reject manual login for Google-only accounts', async () => {
    ...
    await expect(authService.login({ email: 'user@example.com', password: 'password123456' }))
      .rejects
      .toThrow('Please sign in with Google');
  });

  it('should create a Google-only user and establish a normal session', async () => {
    mockGooglePayload({ email: 'new@example.com', sub: 'google-sub-new', email_verified: true });

    const user = authUser({ id: 'new-auth-user-id', passwordHash: null });
    repository.findByProviderIdentity.mockResolvedValue(null);
    repository.findByEmailHash.mockResolvedValue(null);
    repository.createUser.mockResolvedValue(user);
    repository.createSession.mockResolvedValue(session({ authUserId: user.id }));
  ...
  it('should link Google ID to an existing manual account with same verified email', async () => {
    mockGooglePayload({ email: 'existing@example.com', sub: 'google-sub-existing', email_verified: true });

    const existingUser = authUser({ id: 'existing-auth-user-id', passwordHash: 'hash' });
    const linkedUser = authUser({ id: existingUser.id, passwordHash: 'hash' });

    repository.findByProviderIdentity.mockResolvedValue(null);
    repository.findByEmailHash.mockResolvedValue(existingUser);
    repository.linkProviderIdentity.mockResolvedValue(linkedUser);
    repository.createSession.mockResolvedValue(session({ authUserId: linkedUser.id }));
  ...
  it('should reject Google tokens without a verified email', async () => {
    mockGooglePayload({ email: 'unverified@example.com', sub: 'google-sub-unverified', email_verified: false });

    await expect(authService.googleLogin('google-id-token'))
      .rejects
      .toThrow(UnauthorizedException);
  });

  function mockGooglePayload(payload: { email: string; sub: string; email_verified: boolean }) {
