...
export class SessionService {
  ...
  async createSession(authUserId: string, refreshTokenStr: string): Promise<Session> {
    const refreshTokenHash = this.tokenService.hashRefreshToken(refreshTokenStr);
    const refreshTokenFamilyId = randomUUID();
    const expiresAt = new Date(Date.now() + this.tokenService.refreshTokenExpiresInDays * 24 * 60 * 60 * 1000);
  ...
  async rotateSession(refreshTokenStr: string): Promise<{ session: Session, newRefreshTokenStr: string }> {
    const refreshTokenHash = this.tokenService.hashRefreshToken(refreshTokenStr);
    ...
    if (session.status !== 'ACTIVE' || session.expiresAt < new Date()) {
      ...
      throw new UnauthorizedException('Session revoked due to suspicious activity. Please login again.');
    }

    const newRefreshTokenStr = this.tokenService.generateRefreshToken();
    const newRefreshTokenHash = this.tokenService.hashRefreshToken(newRefreshTokenStr);
    const expiresAt = new Date(Date.now() + this.tokenService.refreshTokenExpiresInDays * 24 * 60 * 60 * 1000);
