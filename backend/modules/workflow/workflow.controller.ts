...
export class WorkflowController {
  ...
  @Patch(':id')
  updateBrief(@Param('id') id: string, @Body() dto: Partial<CreateContentAssetDto>) {
  ...
  @Post(':id/actions')
  performAction(@Param('id') id: string, @Body() dto: WorkflowActionDto, @CurrentUser() user: IdentityContext) {
    return this.workflowService.performAction(id, dto, user);
  }

  @Post(':id/recall-submission')
  recallSubmission(@Param('id') id: string, @Body() dto: { actorId: string; submissionId: string }) {
    return this.workflowService.recallSubmission(id, dto);
  }

  @Post(':id/submissions/:submissionId/seen')
  markSubmissionSeen(@Param('id') id: string, @Param('submissionId') submissionId: string, @Body() dto: { actorId: string }) {
