...
import { WorkflowActionDto, WorkflowActionType } from './dto/workflow-action.dto';
...
export class WorkflowService {
  ...
  async getBoard(agencyId: string, query: WorkflowBoardQueryDto = {}, actor?: IdentityContext) {
    const now = new Date();
    const todayStart = new Date(now);
    todayStart.setHours(0, 0, 0, 0);
    const todayEnd = new Date(todayStart);
    todayEnd.setDate(todayEnd.getDate() + 1);
    const canViewBroadWorkflow = actor ? this.canViewBroadWorkflow(actor) : true;

    if (actor && !canViewBroadWorkflow && query.ownerId && query.ownerId !== actor.membershipId) {
      throw new ForbiddenException('You can only filter your own workflow assignments');
    }

    const roleResponsibilities =
      actor && !canViewBroadWorkflow && actor.membershipId
        ? await this.campaignRoleResponsibilities(agencyId, actor.membershipId, query.campaignId)
        : [];
    ...
    const items = assets
      .map((asset) => {
        ...
        const computedRisk = stage === ContentStage.PUBLISHED || stage === ContentStage.ARCHIVED
          ? ContentRisk.ON_TRACK
          : this.computeBoardRisk(workflow?.riskStatus ?? ContentRisk.ON_TRACK, deadlineAt, task?.status, now);
        const latestSubmission = task?.submissions?.[0] ?? null;
        const latestApproval = task?.approvals?.[0] ?? null;
        const lastActivityAt = this.maxIsoDate([asset.updatedAt, workflow?.startedAt, task?.updatedAt, latestSubmission?.createdAt, latestApproval?.createdAt, task?.blockers?.[0]?.createdAt]);
        const isDirectAssignment = task?.owner?.id === actor?.membershipId;
        const isRoleResponsibility = this.isCampaignRoleResponsible(roleResponsibilities, asset.campaignId, stage);
        ...
        return {
          ...
          manager: workflow?.manager ? this.memberSummary(workflow.manager) : null,
          deadlineAt: deadlineAt?.toISOString() ?? null,
          riskStatus: computedRisk,
          taskStatus: stage === ContentStage.PUBLISHED ? TaskStatus.COMPLETED : task?.status ?? null,
          ...
          lastActivityAt,
          responsibility: isDirectAssignment ? 'DIRECT_ASSIGNMENT' : isRoleResponsibility ? 'CAMPAIGN_ROLE' : null
        };
      })
      .filter((item) => !query.ownerId || item.owner?.membershipId === query.ownerId || (!canViewBroadWorkflow && query.ownerId === actor?.membershipId && item.responsibility === 'CAMPAIGN_ROLE'))
    ...
    const summary = {
      active: items.filter((item) => !['PUBLISHED', 'ARCHIVED'].includes(item.stage)).length,
      waitingReview: items.filter((item) => item.taskStatus === TaskStatus.WAITING_REVIEW || item.approvalStatus === ApprovalStatus.PENDING).length,
      blocked: items.filter((item) => item.hasActiveBlocker || item.riskStatus === ContentRisk.BLOCKED).length,
      overdue: items.filter((item) => item.riskStatus === ContentRisk.OVERDUE).length,
      dueToday: items.filter((item) => item.deadlineAt && new Date(item.deadlineAt) >= todayStart && new Date(item.deadlineAt) < todayEnd).length
    };
  ...
  async createContentAsset(dto: CreateContentAssetDto) {
    const result = await this.prisma.$transaction(async (tx) => {
      const displayCode = await this.generateDisplayCode(tx, dto.agencyId, dto.type);
  ...
  async advanceStage(id: string, dto: { actorId: string; toStage: ContentStage; reason?: string }) {
    const contentAsset = await this.ensureContentAssetExists(id);
    const workflowInstance = await this.ensureActiveWorkflowInstanceForContentAsset(id);
  ...
  async assign(id: string, dto: AssignContentDto) {
    const contentAsset = await this.ensureContentAssetExists(id);
    const result = await this.prisma.$transaction(async (tx) => {
      const workflowInstance = await this.ensureWorkflowInstance(tx, contentAsset.agencyId, id);
      const previousTask = workflowInstance.currentTaskId
        ? await tx.workflowTask.findUnique({ where: { id: workflowInstance.currentTaskId } })
        : null;
      ...
      await this.createTaskNotification(tx, contentAsset.agencyId, dto.assigneeId, {
  ...
  async submit(id: string, dto: SubmitWorkDto) {
    await this.ensureContentAssetExists(id);
    const task = await this.ensureTaskBelongsToContentAsset(dto.workflowTaskId, id);

    if (task.ownerMembershipId !== dto.actorId) {
      throw new BadRequestException('Only the current task owner can submit this work');
    }

    this.ensureSubmissionHasContent(dto);

    const latestSubmission = await this.prisma.submission.findFirst({
      where: { workflowTaskId: dto.workflowTaskId, submissionType: dto.submissionType },
    ...
    await this.createMembershipNotification(task.agencyId, task.workflowInstance.managerMembershipId, {
  ...
  async markSubmissionSeen(id: string, submissionId: string, dto: { actorId: string }) {
    await this.ensureContentAssetExists(id);
    ...
    if (!submission || submission.workflowTask.workflowInstance.contentAssetId !== id) {
  ...
  async recallSubmission(id: string, dto: { actorId: string; submissionId: string }) {
    await this.ensureContentAssetExists(id);
    const submission = await this.prisma.submission.findUnique({
    ...
    if (submission.submittedBy !== dto.actorId || submission.status !== SubmissionStatus.SUBMITTED || submission.seenAt) {
  ...
  async performAction(id: string, dto: WorkflowActionDto, actor: IdentityContext) {
    ...
    const stage = task.workflowStep?.stage ?? await this.latestWorkflowStage(task.workflowInstanceId);
    ...
    if (dto.action === WorkflowActionType.SUBMIT_FOR_REVIEW) {
      return this.submitForReviewAction(id, task, stage, dto, actor.membershipId);
    }

    if (dto.action === WorkflowActionType.APPROVE || dto.action === WorkflowActionType.ACCEPT_HANDOVER) {
      return this.approveAction(id, task, stage, dto, actor.membershipId);
    }

    if (dto.action === WorkflowActionType.REQUEST_CHANGES || dto.action === WorkflowActionType.REJECT) {
  ...
  async approve(id: string, dto: ApproveContentDto) {
    ...
    const existing = await this.prisma.approval.findUnique({
      where: { agencyId_idempotencyKey: { agencyId: task.agencyId, idempotencyKey: dto.idempotencyKey } }
    });
    ...
    const autoAdvance = dto.nextOwnerId && dto.nextStage && dto.nextDeadlineAt
      ? null
    ...
    if (nextStage && !canTransition(task.workflowStep?.stage ?? null, nextStage)) {
      const latestStage = await this.latestWorkflowStage(task.workflowInstanceId);
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.workflowTask.update({
        where: { id: task.id },
        data: { status: TaskStatus.COMPLETED, completedAt: new Date(), version: { increment: 1 } }
      });
      ...
      if (autoAdvance?.completeWorkflow) {
        ...
        await tx.contentAsset.update({
          where: { id },
          data: { status: ContentAssetStatus.COMPLETED, version: { increment: 1 } }
        });

        await tx.publishingSchedule.updateMany({
          where: { agencyId: task.agencyId, contentAssetId: id, status: { in: ['PLANNED', 'SCHEDULED'] } },
          data: { status: 'READY', riskStatus: ContentRisk.ON_TRACK, version: { increment: 1 } }
        });
        ...
        await this.publishWithinTransaction(tx, DomainEvents.WorkflowStageChanged, {
          agencyId: task.agencyId,
          actorId: dto.actorId,
  ...
  async requestChanges(id: string, dto: RequestChangesDto) {
    ...
    if (dto.returnToStage && !canTransition(task.workflowStep?.stage ?? null, dto.returnToStage)) {
      throw new BadRequestException('Invalid workflow transition');
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.createTaskNotification(tx, task.agencyId, dto.returnToOwnerId, {
  ...
  private async submitForReviewAction(
    ...
    task: Prisma.WorkflowTaskGetPayload<{
      include: {
        workflowInstance: {
          include: {
            contentAsset: { include: { campaign: { include: { teamAssignments: true, publishingSchedules: true } }, client: true } }
          }
        },
        workflowStep: true,
        owner: true
    ...
    if (!this.canSubmitCurrentStage(actorMembershipId, task, stage)) {
      throw new ForbiddenException('Only the current task owner can submit this work');
    }
    ...
    if (!reviewStage) {
      throw new BadRequestException(`${this.stageLabel(stage)} cannot be submitted for review`);
    }

    const submissionType = this.submissionTypeFor(stage);
    const reviewOwnerId = this.reviewOwnerForStage(stage, task.workflowInstance);
    const reviewStepId = await this.workflowStepIdForStage(task.agencyId, task.workflowInstanceId, reviewStage);
    const latestSubmission = await this.prisma.submission.findFirst({
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      if (task.ownerMembershipId !== actorMembershipId) {
        await tx.workflowTask.update({
          where: { id: task.id },
          data: { ownerMembershipId: actorMembershipId, version: { increment: 1 } },
        });
      ...
      await this.createTaskNotification(tx, task.agencyId, reviewOwnerId, {
        ...
        body: dto.comment || `${this.stageLabel(stage)} was submitted for review.`,
        eventType: DomainEvents.SubmissionCreated,
      });
  ...
  private canSubmitCurrentStage(
    ...
    return task.workflowInstance.contentAsset.campaign.teamAssignments.some((assignment) => (
  ...
  private async approveAction(
    ...
    if (task.ownerMembershipId !== actorMembershipId && !this.canReviewWorkflow(actorMembershipId, task.workflowInstance)) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (next.completeWorkflow) {
        ...
        await tx.publishingSchedule.updateMany({
          where: { agencyId: task.agencyId, contentAssetId, status: { in: ['PLANNED', 'SCHEDULED'] } },
          data: { status: 'READY', riskStatus: ContentRisk.ON_TRACK, version: { increment: 1 } },
        });
      } else if (next.stage && next.ownerId && next.deadlineAt) {
        const nextStepId = await this.workflowStepIdForStage(task.agencyId, task.workflowInstanceId, next.stage);
        const nextTask = await tx.workflowTask.create({
          data: {
            agencyId: task.agencyId,
            workflowInstanceId: task.workflowInstanceId,
        ...
        await tx.assignmentHistory.create({
          data: {
            ...
            workflowStepId: nextStepId,
            changedByMembershipId: actorMembershipId,
            reason: dto.action === WorkflowActionType.ACCEPT_HANDOVER ? 'Handover accepted' : 'Approved',
          },
        });

        await this.createTaskNotification(tx, task.agencyId, next.ownerId, {
          title: `New task: ${this.stageLabel(next.stage)}`,
          body: dto.comment || `Content moved to ${this.stageLabel(next.stage)}.`,
          eventType: DomainEvents.ContentAssigned,
        });
      ...
      if (dto.action === WorkflowActionType.ACCEPT_HANDOVER) {
        await this.publishWithinTransaction(tx, DomainEvents.SubmissionAccepted, {
          agencyId: task.agencyId,
          actorId: actorMembershipId,
          aggregateId: task.id,
          aggregateType: 'WorkflowTask',
  ...
  private async returnAction(
    ...
    if (!previous.stage || !previous.ownerId) {
      throw new BadRequestException(`${this.stageLabel(stage)} cannot be returned`);
    }
    const previousStepId = await this.workflowStepIdForStage(task.agencyId, task.workflowInstanceId, previous.stage);
    const approvalStatus = dto.action === WorkflowActionType.REJECT ? ApprovalStatus.REJECTED : ApprovalStatus.CHANGES_REQUESTED;
    const eventType = dto.action === WorkflowActionType.REJECT ? DomainEvents.ApprovalRejected : DomainEvents.ChangesRequested;
    ...
    const approval = await this.prisma.$transaction(async (tx) => {
      ...
      await this.createTaskNotification(tx, task.agencyId, previous.ownerId, {
        title: dto.action === WorkflowActionType.REJECT ? 'Work rejected' : 'Changes requested',
        body: dto.comment || dto.reason || 'Please review the requested changes.',
        eventType,
      });
      ...
      if (stage === ContentStage.EDITOR_INTAKE) {
        await this.publishWithinTransaction(tx, DomainEvents.SubmissionRejected, {
          agencyId: task.agencyId,
          actorId: actorMembershipId,
          aggregateId: task.id,
          aggregateType: 'WorkflowTask',
  ...
  private async currentTaskForAction(contentAssetId: string, agencyId: string) {
    const workflowInstance = await this.prisma.workflowInstance.findFirst({
      where: { agencyId, contentAssetId, status: WorkflowInstanceStatus.ACTIVE },
      include: {
        currentTask: {
          include: {
            workflowStep: true,
  ...
  private submissionTypeFor(stage: ContentStage) {
    ...
    if (!submissionType) {
      throw new BadRequestException(`${this.stageLabel(stage)} does not support submissions`);
    }
    return submissionType;
  }

  private ensureSubmissionHasContent(dto: { body?: string; externalLink?: string }) {
    if (!dto.body?.trim() && !dto.externalLink?.trim()) {
      throw new BadRequestException('Add a link or note before submitting this work');
    }
  }
  ...
  private reviewOwnerForStage(
    ...
    workflowInstance: {
      ...
      contentAsset: {
        campaign: {
          teamAssignments: Array<{ membershipId: string; assignmentRole: CampaignAssignmentRole }>;
        };
      };
    }
  ) {
  ...
  private async nextStageAfterApproval(
    ...
    const publishingAt = task.workflowInstance.contentAsset.campaign.publishingSchedules[0]?.scheduledAt ?? task.deadlineAt;
    const teamAssignments = task.workflowInstance.contentAsset.campaign.teamAssignments;

    if (stage === ContentStage.MANAGER_SCRIPT_REVIEW) {
      return {
        stage: ContentStage.SHOOT,
        ownerId: this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.DOP, 'Assign a DOP before approving script work'),
    ...
    if (stage === ContentStage.EDITOR_INTAKE) {
      return {
        ...
        ownerId: this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.EDITOR, 'Assign an editor before accepting footage'),
        deadlineAt: this.relativeDeadline(publishingAt, -1, 18),
      };
    }

    if (stage === ContentStage.MANAGER_EDIT_REVIEW || stage === ContentStage.CLIENT_APPROVAL) {
    ...
    if (stage === ContentStage.SHOOT) {
      return {
        ...
        ownerId: this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.EDITOR, 'Assign an editor before approving shoot work'),
        deadlineAt: this.relativeDeadline(publishingAt, -1, 12),
      };
    }

    const autoAdvance = await this.resolveAutomaticAdvance(task, contentAssetId);
    if (autoAdvance?.completeWorkflow) return { completeWorkflow: true };
    if (autoAdvance?.nextStage && autoAdvance.nextOwnerId && autoAdvance.nextDeadlineAt) {
      return {
    ...
    throw new BadRequestException(`${this.stageLabel(stage)} cannot be approved`);
  }
  ...
  private async previousStageForReturn(
    ...
    if (stage === ContentStage.MANAGER_SCRIPT_REVIEW) {
      return {
        ...
        ownerId: await this.latestSubmitterFor(task.workflowInstanceId, SubmissionType.SCRIPT)
          ?? this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.WRITER, 'Assign a writer before requesting script changes'),
    ...
    if (stage === ContentStage.EDITOR_INTAKE) {
      return {
        stage: ContentStage.SHOOT,
        ownerId: await this.latestSubmitterFor(task.workflowInstanceId, SubmissionType.RAW_FOOTAGE)
          ?? this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.DOP, 'Assign a DOP before rejecting footage handover'),
        deadlineAt: this.relativeDeadline(publishingAt, -2, 18),
      };
    }
    ...
    if (stage === ContentStage.MANAGER_EDIT_REVIEW) {
      return {
        stage: ContentStage.EDITING,
        ownerId: await this.latestSubmitterFor(task.workflowInstanceId, SubmissionType.FINAL_CUT)
          ?? this.requiredCampaignAssignee(teamAssignments, CampaignAssignmentRole.EDITOR, 'Assign an editor before requesting edit changes'),
        deadlineAt: this.relativeDeadline(publishingAt, -1, 18),
      };
    }
  ...
  private async latestSubmitterFor(workflowInstanceId: string, submissionType: SubmissionType) {
  ...
  private async workflowStepIdForStage(agencyId: string, workflowInstanceId: string, stage: ContentStage) {
  ...
  private async ensureContentAssetExists(id: string) {
    const contentAsset = await this.prisma.contentAsset.findUnique({ where: { id } });
  ...
  private async resolveAutomaticAdvance(
    task: { agencyId: string; workflowInstanceId: string; workflowStep?: { stage: ContentStage } | null },
    contentAssetId: string
  ) {
    const currentStage = task.workflowStep?.stage ?? await this.latestWorkflowStage(task.workflowInstanceId);
    ...
    const publishingAt = context.campaign.publishingSchedules[0]?.scheduledAt ?? new Date();

    if (currentStage === ContentStage.WRITING) {
      return {
        nextStage: ContentStage.SHOOT,
        nextOwnerId: this.requiredCampaignAssignee(context.campaign.teamAssignments, CampaignAssignmentRole.DOP, 'Assign a DOP before approving script work'),
    ...
    if (currentStage === ContentStage.SHOOT) {
      return {
        ...
        nextOwnerId: this.requiredCampaignAssignee(context.campaign.teamAssignments, CampaignAssignmentRole.EDITOR, 'Assign an editor before approving shoot work'),
    ...
    if (currentStage === ContentStage.EDITOR_INTAKE) {
      return {
        ...
        nextOwnerId: this.requiredCampaignAssignee(context.campaign.teamAssignments, CampaignAssignmentRole.EDITOR, 'Assign an editor before accepting footage'),
        nextDeadlineAt: this.relativeDeadline(publishingAt, -1, 18).toISOString()
      };
    }

    if (currentStage === ContentStage.EDITING || currentStage === ContentStage.MANAGER_EDIT_REVIEW) {
      return { completeWorkflow: true };
    }
  ...
  private async campaignRoleResponsibilities(agencyId: string, membershipId: string, campaignId?: string) {
  ...
  private isCampaignRoleResponsible(
    ...
    return responsibilities.some((responsibility) => responsibility.campaignId === campaignId && responsibility.stages.includes(stage));
  }
  ...
  private requiredCampaignAssignee(
    assignments: Array<{ membershipId: string; assignmentRole: CampaignAssignmentRole }>,
    assignmentRole: CampaignAssignmentRole,
    message: string
  ) {
    const assignment = assignments.find((item) => item.assignmentRole === assignmentRole);
    if (!assignment) {
  ...
  private relativeDeadline(publishingAt: Date, daysOffset: number, hour: number) {
  ...
  private async publishWithinTransaction(
    ...
    if (typeof this.eventBus.publishWithinTransaction !== 'function') return null;
  ...
  private async ensureActiveWorkflowInstanceForContentAsset(contentAssetId: string) {
    const workflowInstance = await this.prisma.workflowInstance.findFirst({
  ...
  private async ensureTaskBelongsToContentAsset(workflowTaskId: string, contentAssetId: string) {
    ...
    if (!task || task.workflowInstance.contentAssetId !== contentAssetId) {
      throw new NotFoundException('Workflow task not found for this content asset');
    }
  ...
  private computeBoardRisk(currentRisk: ContentRisk, deadlineAt: Date | null | undefined, taskStatus: TaskStatus | null | undefined, now: Date) {
    if (currentRisk === ContentRisk.BLOCKED || taskStatus === TaskStatus.BLOCKED) {
  ...
  private memberSummary(member: { id: string; user?: { name: string | null } | null; role?: { displayName: string } | null }) {
    return {
