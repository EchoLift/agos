...
import { CalendarEventsQueryDto, CalendarScope } from './dto/calendar-events-query.dto';
...
export class CalendarService {
  ...
  async getEvents(query: CalendarEventsQueryDto, actor: IdentityContext) {
    ...
    const scope = this.resolveScope(query.scope ?? this.defaultScope(actor), actor);
    ...
    const accessibleCampaignIds = await this.resolveAccessibleCampaignIds(scope, agencyId, membershipId, actor, query);
    ...
    const [taskEvents, publishingEvents] = await Promise.all([
      !eventTypes.length || eventTypes.some((type) => ['WORKFLOW_TASK', 'SHOOT', 'REVIEW', 'APPROVAL'].includes(type))
        ? this.getWorkflowTaskEvents({ agencyId, membershipId, roleKeys, scope, from, to, campaignId: query.campaignId, memberId: query.memberId, statuses: taskStatuses, rawStatuses: statuses, accessibleCampaignIds })
        : Promise.resolve([]),
      !eventTypes.length || eventTypes.includes('PUBLISHING')
        ? this.getPublishingEvents({ agencyId, membershipId, roleKeys, scope, from, to, campaignId: query.campaignId, statuses: publishingStatuses, rawStatuses: statuses, platforms: publishingPlatforms, rawPlatforms: platforms, accessibleCampaignIds })
        : Promise.resolve([]),
    ]);

    const events = [...taskEvents, ...publishingEvents].sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime());
    ...
    return {
      ...
      summary: {
        total: events.length,
        assignedToMe: events.filter((event) => event.assignedMembershipIds.includes(membershipId)).length,
        publishing: events.filter((event) => event.eventType === 'PUBLISHING').length,
        overdue: events.filter((event) => event.riskStatus === ContentRisk.OVERDUE).length,
      },
    };
  }
  ...
  private async getWorkflowTaskEvents(params: {
    ...
    const roleResponsibilities =
      params.scope === 'MY_SCHEDULE'
        ? await this.campaignRoleResponsibilities(params.agencyId, params.membershipId, params.campaignId)
        : [];
    ...
    const where: any = {
      ...
      ...(params.campaignId ? { workflowInstance: { contentAsset: { campaignId: params.campaignId } } } : {}),
    };
    ...
    if (params.scope === 'MY_SCHEDULE') {
      ...
      where.workflowInstance = {
        ...
        contentAsset: {
          ...
          campaignId: this.campaignIdFilter(params.campaignId, params.accessibleCampaignIds ?? []),
        },
      };
    ...
    return tasks.map((task) => {
      ...
      return {
        ...
        sourceId: task.id,
        eventType,
        title: task.displayName || `${this.labelize(eventType)} ${contentAsset.displayCode}`,
        ...
        campaign: { id: contentAsset.campaign.id, name: contentAsset.campaign.name },
        client: { id: contentAsset.client.id, name: contentAsset.client.displayName ?? contentAsset.client.name },
        contentAsset: { id: contentAsset.id, displayCode: contentAsset.displayCode, title: contentAsset.title },
        visibility: isDirectAssignment ? 'DIRECT_ASSIGNMENT' : isRoleResponsibility ? 'CAMPAIGN_ROLE' : 'DIRECT_ASSIGNMENT',
        status: task.status,
        riskStatus,
        owner: task.owner ? { membershipId: task.owner.id, name: task.owner.user.name ?? 'Unassigned' } : null,
  ...
  private async getPublishingEvents(params: {
    ...
    if ((params.rawStatuses.length && !params.statuses.length) || (params.rawPlatforms.length && !params.platforms.length)) return [];
    ...
    const where: any = {
      ...
      ...(params.statuses.length ? { status: { in: params.statuses } } : {}),
      ...(params.platforms.length ? { platform: { in: params.platforms } } : {}),
    };

    if (params.scope === 'MY_SCHEDULE') {
      const campaignIds = await this.campaignIdsForDirectPublishing(params.agencyId, params.membershipId);
      where.campaignId = this.campaignIdFilter(params.campaignId, campaignIds);
    } else if (params.scope === 'MY_TEAM' || params.scope === 'CAMPAIGN') {
      where.campaignId = this.campaignIdFilter(params.campaignId, params.accessibleCampaignIds ?? []);
    }
    ...
    return slots.map((slot) => ({
      ...
      client: { id: slot.campaign.client.id, name: slot.campaign.client.displayName ?? slot.campaign.client.name },
      contentAsset: slot.contentAsset ? { id: slot.contentAsset.id, displayCode: slot.contentAsset.displayCode, title: slot.contentAsset.title } : null,
      ...
      reason: params.roleKeys.includes('SOCIAL_MEDIA_MANAGER') ? 'Publishing work for my role' : this.labelize(params.scope),
    }));
  }

  private async resolveAccessibleCampaignIds(scope: CalendarScope, agencyId: string, membershipId: string, actor: IdentityContext, query: CalendarEventsQueryDto) {
    if (scope === 'AGENCY') {
    ...
    if (scope === 'CAMPAIGN') {
      ...
      const canAccess = await this.canAccessCampaign(agencyId, membershipId, actor, query.campaignId);
    ...
    const assignments = await this.prisma.campaignTeamAssignment.findMany({
      where: {
        agencyId,
        membershipId,
        assignmentRole: { in: [CampaignAssignmentRole.CAMPAIGN_MANAGER, CampaignAssignmentRole.RELATIONSHIP_MANAGER] },
      },
  ...
  private async canAccessCampaign(agencyId: string, membershipId: string, actor: IdentityContext, campaignId: string) {
  ...
  private async campaignIdsForDirectPublishing(agencyId: string, membershipId: string) {
    const assignments = await this.prisma.campaignTeamAssignment.findMany({
      where: {
        agencyId,
        membershipId,
  ...
  private stagesForCampaignRole(role: CampaignAssignmentRole) {
    const stagesByRole: Record<CampaignAssignmentRole, string[]> = {
      [CampaignAssignmentRole.CAMPAIGN_MANAGER]: ['MANAGER_SCRIPT_REVIEW', 'MANAGER_EDIT_REVIEW', 'CLIENT_APPROVAL'],
      [CampaignAssignmentRole.RELATIONSHIP_MANAGER]: ['MANAGER_SCRIPT_REVIEW', 'MANAGER_EDIT_REVIEW', 'CLIENT_APPROVAL'],
      ...
      [CampaignAssignmentRole.AGENCY_APPROVER]: ['MANAGER_SCRIPT_REVIEW', 'MANAGER_EDIT_REVIEW'],
    };
  ...
  private defaultScope(actor: IdentityContext): CalendarScope {
    ...
    if (this.roleKeys(actor).some((role) => role === 'MANAGER')) return 'MY_TEAM';
    return 'MY_SCHEDULE';
  }

  private resolveScope(scope: CalendarScope, actor: IdentityContext): CalendarScope {
    if (this.allowedScopes(actor).includes(scope)) return scope;
    throw new ForbiddenException(`${this.labelize(scope)} is not available for your current role`);
  }
  ...
  private ensureAgencyScope(actor: IdentityContext) {
    if (!this.isOwnerOrManager(actor)) {
      throw new ForbiddenException('Only owners and managers can view the agency calendar');
    }
  }

  private ensureMemberFilterAccess(memberId: string | undefined, membershipId: string, actor: IdentityContext) {
    if (!memberId || memberId === membershipId) return;
    if (!this.isOwnerOrManager(actor)) {
      throw new ForbiddenException('Only owners and managers can filter another member calendar');
    }
  }

  private isOwnerOrManager(actor: IdentityContext) {
    return this.roleKeys(actor).some((role) => role === 'OWNER' || role === 'MANAGER');
  }
  ...
  private resolveRange(query: CalendarEventsQueryDto) {
    ...
    const to = query.to ? new Date(query.to) : new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    if (from > to) {
  ...
  private taskRisk(status: TaskStatus, deadlineAt: Date) {
    ...
    if (deadlineAt < new Date() && status !== TaskStatus.COMPLETED) return ContentRisk.OVERDUE;
  ...
  private publishingRisk(status: PublishingStatus, scheduledAt: Date) {
    ...
    if (status === PublishingStatus.PUBLISHED || status === PublishingStatus.CANCELLED) return ContentRisk.ON_TRACK;
    if (scheduledAt < new Date()) return ContentRisk.OVERDUE;
    return ContentRisk.ON_TRACK;
  }

  private roleKeys(actor: IdentityContext) {
    return [actor.role, ...(actor.roles ?? [])].filter(Boolean).map((role) => role!.toUpperCase());
  }

  private membershipRoleKeys(membership: { role?: any; roles?: any[] }) {
    const primary = membership.role?.systemRole?.key;
    const additional = membership.roles?.map((item) => item.role?.systemRole?.key).filter(Boolean) ?? [];
  ...
  private campaignIdFilter(requestedCampaignId: string | undefined, allowedCampaignIds: string[]) {
    if (requestedCampaignId) {
      return { in: allowedCampaignIds.includes(requestedCampaignId) ? [requestedCampaignId] : [] };
    }
    return { in: allowedCampaignIds };
  }

  private enumValues<T extends Record<string, string>>(source: T, values: string[]) {
