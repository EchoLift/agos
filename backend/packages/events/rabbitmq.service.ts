...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connect() {
    ...
    try {
      ...
      await this.channel.assertExchange("agency_os.events", "topic", {
        durable: true,
      });
      this.logger.log(
        "Connected to RabbitMQ and asserted exchange: agency_os.events",
      );
    } catch (err) {
  ...
  async subscribe(
    queueName: string,
    routingKey: string,
    handler: (msg: amqplib.ConsumeMessage | null) => Promise<void>,
  ) {
    if (!this.channel) {
