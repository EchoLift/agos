...
## Ciphertext Formats & Encryption Standardization Note
- **Historical Formats**:
  - **Format 1 (Standard)**: `${ivHex}:${authTagHex}:${encryptedHex}`, generated by `CryptoService` (`@modules/auth/services/crypto.service.ts`) using a SHA-256 key derivation (`createHash("sha256").update(secret).digest()`). Used by `AuthService`, `OrganizationService`, `UserService`, `ClientContactService`, and DB seeders.
  - **Format 2 (Legacy Base64)**: `base64(iv + tag + cipher)`, generated by early versions of `FieldCryptoService` (`@packages/crypto/field-crypto.service.ts`).
- **Guidelines**:
  - All **future write code** MUST standardize on **Format 1** (`CryptoService` colon-separated format).
