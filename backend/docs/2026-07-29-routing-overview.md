...
- Loads available agency roles from `GET /organizations/roles` and sends `X-Agency-Id` on tenant-scoped requests.
...
- Returns the list of roles available to the current agency.
- Used by the team invite page to render role selection for the hiring team.
...
- `Invitation.mobileNumber` is persisted in the database as part of the latest schema sync.
